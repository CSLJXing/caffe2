...
from caffe2.python import data_workers
...
class ItemReader:
    def __init__(self, fname, shuffle=False):
        self._shuffle = shuffle
        self._lock = Lock()
        self._iter = 0
        with open(fname) as fid:
            self._items = [_ for _ in fid]
        if shuffle:
            ShuffleList(self._items)

    def getNext(self):
        self._lock.acquire()
        item_ind = self._iter
        self._iter += 1
        if self._iter == len(self._items):
            self._iter = 0
            if self._shuffle:
                ShuffleList(self._items)
        self._lock.release()
        return self._items[self._iter]


def AddImageInput(model, item_reader, img_root, batch_size,
               img_size, label_dim, input_source_name):
    def data_fetcher_fun(worker_id, batch_size):
        img_batch = np.empty((batch_size, 3, img_size, img_size), dtype=np.float32)
        label_batch = np.empty((batch_size, label_dim), dtype=np.int32)
        for idx in range(batch_size):
            item = item_reader.getNext()
            sp = item.strip().split(' ')
            img = imresize(
                imread(img_root + sp[0], mode='RGB'),
                (img_size, img_size)
            )
            img_batch[idx] = img[:,:, (2,1,0)].transpose((2,0,1))
            label_batch[idx] = np.float32([float(_) for _ in sp[1:]])
        return img_batch, label_batch.squeeze()

    data_workers.init_data_input_workers(
        model,
        ['data', 'label'],
        data_fetcher_fun,
...
def Train(args):
    ...
    def create_model_fn(model, loss_scale):
    ...
    train_reader = ItemReader(
        fname=args.train_list,
        shuffle=True
    )

    def train_data_input_fn(model):
        AddImageInput(
            model,
            train_reader,
    ...
    data_parallel_model.Parallelize(
        train_model,
        input_builder_fun=train_data_input_fn,
        forward_pass_builder_fun=create_model_fn,
    ...
    if (args.test_list is not None):
        ...
        # Input. Note that the reader must be shared with all GPUS.
        test_reader = ItemReader(
            fname=args.test_list,
            shuffle=False
        )
    
        def test_data_input_fn(model):
            AddImageInput(
                model,
                test_reader,
                ...
                img_size=args.image_size,
                label_dim=1,
                input_source_name='test'
            )

        data_parallel_model.Parallelize(
            test_model,
            input_builder_fun=test_data_input_fn,
            forward_pass_builder_fun=create_model_fn,
    ...
    data_workers.global_coordinator.start()
...
def main():
    ...
    parser.add_argument("--train_list", type=str, default=None,
                        ...
                        required=True)
    parser.add_argument("--train_img_root", type=str, default="")
    parser.add_argument("--test_list", type=str, default=None,
                        help="Path to test data")
    parser.add_argument("--test_img_root", type=str, default="")
